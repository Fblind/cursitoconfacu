...
function generarFilasDeGastos() {
  let filas = "";
  for (let index = 0; index < listaDeGastos.length; index++) {
    const gasto = listaDeGastos[index];
    filas = filas + `
    <tr>
      <td>${gasto.fecha}</td>
      <td>${gasto.monto}</td>
...
function mostrarGastosEnPantalla(button, htmlAMostrar) {
  const elemento = document.createElement("div");
  elemento.id = "gastos";
  elemento.innerHTML = htmlAMostrar; 
  button.insertAdjacentElement("afterend", elemento);
}

function mostrarGastos(evento) {
  console.log(listaDeGastos);
  // volver a ver el refactor de habilitar [DONE]
  ...
  if (listaDeGastos.length === 0) {
    ...
    `;
    mostrarGastosEnPantalla(evento.target, htmlAMostrar);
  } else {
    htmlAMostrar = `
    <table>
      <thead>
